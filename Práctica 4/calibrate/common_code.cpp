                                  ...
                                  const char *wname)
{
    ...
    if (was_found)
    {
        ...
        if (wname != nullptr)
        {
            ...
            cv::drawChessboardCorners(copia,board_size,corner_points,was_found);
            cv::imshow(wname,copia);
                            ...
                            std::vector<cv::Mat> *tvecs)
{
    ...
    if (rvecs) *rvecs = Rvecs;

    if (tvecs) *tvecs = Tvecs;
    //
                                 ...
                                 double fps)
{
    ...
    cv::Mat map1, map2, frame;

    input_stream.read(frame);
    cv::initUndistortRectifyMap(camera_matrix, dist_coeffs, cv::Mat(), cv::Mat(), frame.size(), CV_32FC2, map1, map2);
    ...
    {
        cv::Mat output;
        cv::remap(frame, output, map1, map2, interp);
