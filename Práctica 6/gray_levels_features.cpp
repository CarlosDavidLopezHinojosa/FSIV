...
fsiv_extract_01_normalized_graylevels (cv::Mat const& img)
{
    ...
    cv::normalize(gray_img, normalized, 0, 1, cv::NORM_MINMAX);

    feature = normalized.reshape(1, 1);
    //
...
fsiv_extract_mean_stddev_normalized_gray_levels(cv::Mat const& img)
{
    ...
    cv::meanStdDev(gray_img, mean, stddev);

    gray_img = (gray_img - mean[0]) / stddev[0];

    feature = gray_img.reshape(1, 1);
    //
    CV_Assert(feature.rows==1);
    CV_Assert(feature.type()==CV_32FC1);
    return feature;
