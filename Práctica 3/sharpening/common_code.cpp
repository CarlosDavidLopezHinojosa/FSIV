...
cv::Mat fsiv_create_dog_filter(int r1, int r2)
{
    ...
    cv::Mat g1_exp = fsiv_fill_expansion(g1, r2 - r1);
    ret_v = g2 - g1_exp;
    //
...
fsiv_create_sharpening_filter(const int filter_type, int r1, int r2)
{
    ...
    cv::Mat laplacian = (filter_type == 0) ? fsiv_create_lap4_filter() :
                        (filter_type == 1) ? fsiv_create_lap8_filter() :
                        fsiv_create_dog_filter(r1, r2);

    cv::Mat impulse = cv::Mat::zeros(laplacian.rows, laplacian.cols, CV_32FC1);
    impulse.at<float>(impulse.rows / 2, impulse.cols / 2) = 1.0;

    filter = -laplacian + impulse;
    //
                      ...
                      int r1, int r2, bool circular)
{
    ...
    cv::filter2D(expanded_in, out, -1, filter, cv::Point(-1, -1), 0, cv::BORDER_ISOLATED);
